...
- Modified the `__init__` method of `ClassInheritsCollector` to accept `base_class` and `exclude_files_regex` as parameters.
- Added a `collect` method to `ClassInheritsCollector` that iterates over the provided paths, reads the file content, parses it into an AST, and calls the `match_in_file` method to collect the code elements.
- Simplified the file matching logic by checking the exclude regex pattern before reading the file content.
- Commented out the `annotate_parent` method as it is not used in the current implementation.