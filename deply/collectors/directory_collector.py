...
class DirectoryCollector(BaseCollector):
    def __init__(self, config: dict, paths: List[str], exclude_files: List[str]):
        ...
        self.class_name_regex_collector = ClassNameRegexCollector(config, paths, exclude_files) if self.class_name_regex_pattern else None
        self.class_inherits_collector = ClassInheritsCollector(config, paths, exclude_files) if self.base_class else None
    ...
    def get_elements_in_file(self, file_path: Path) -> Set[CodeElement]:
        ...
        if not self.element_type or self.element_type == 'class':
            if self.class_name_regex_collector:
                elements.update(self.class_name_regex_collector.match_in_file(tree, file_path))
            elif self.class_inherits_collector:
                elements.update(self.class_inherits_collector.match_in_file(tree, file_path))
            else:
